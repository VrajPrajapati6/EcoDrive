...
const JWT_SECRET =
  process.env.JWT_SECRET || "supersecret_ecodrive_jwt_key_2026_hackathon";
...
async function register(req, res) {
  const { fullName, email, password, role, organizationId, employeeId, phone } =
    req.body;
  ...
  if (!fullName || !email || !password || !role || !organizationId) {
    return res
      ...
      .json({
        error:
          "Please provide all required fields: Full Name, Email, Password, Role, and Organization.",
      });
  }
  ...
  try {
    ...
    const orgRes = await pool.query(
      "SELECT id, name, domain, code FROM organizations WHERE id = $1",
      [organizationId],
    );
...
async function login(req, res) {
  ...
  if (!email || !password || !role || !organizationId) {
    return res
      .status(400)
      .json({
        error: "Please provide Email, Password, Role, and Organization.",
      });
  }
  ...
  try {
    ...
    const userRes = await pool.query(
      "SELECT id, email, password_hash, full_name, role, organization_id, employee_id, phone, is_active FROM users WHERE email = $1",
      [email],
    );
  ...
  try {
    ...
    if (!row.isActive) {
      return res
        .status(403)
        .json({
          error: "Your account has been deactivated or is pending approval.",
        });
    }
    ...
    const user = {
      ...
      organization: row.orgId
        ? {
            ...
            fuelCost: parseFloat(row.orgFuelCost) || 96.5,
            costPerKm: parseFloat(row.orgCostPerKm) || 8.0,
            travelCostOperational:
              parseFloat(row.orgTravelCostOperational) || 2.5,
          }
        : null,
    };
